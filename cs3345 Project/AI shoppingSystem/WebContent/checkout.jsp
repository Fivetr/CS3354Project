...
<body>
	<h2>CHECK OUT   <i class="fas fa-money-check-alt"></i></h2>
	<br>
    ...
    <br>
		<%
			ResultSet Result3 = statement.executeQuery("select *from users where name ='"+user+"'"); //get product from database
			while(Result3.next()){
		%>
	...
	<div class = "email">
		<h4>Email: <%=Result3.getString("email") %><a href="editemail.jsp">Edit Email <i class="fas fa-edit"></i></a></h4>
	</div>
	...
	<%}
	 ...
	 <br>
	 <br>
	 <h5>CheckOut  <i class="far fa-credit-card"></i></h5>
	 <form action = "checkoutUpdate.jsp" method = "post">
		 	<div class="left">
			<h3>CardNumbers: <input type="tel" placeholder="1234-5678-9000" pattern="[0-9]{4}-[0-9]{4}-[0-9]{4}" required></h3>
			</div>
			
			<div class="right">
			<h3>ExpiryDate: <input type="date" value="<?= date('Y-m-d', time()); ?>"  required/></h3>
			</div> 
		
			<div class="left">
			<h3>CVC: <input type="tel" placeholder="123" pattern="[0-9]{3}" required></h3>
			</div>
